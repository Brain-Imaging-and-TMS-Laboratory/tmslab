...
- **File Structure:**
  ...
  - `Dockerfile`, `docker-compose.yml`, `docker-compose-slim.yml` – upstream al-folio Docker configs (NOT used in this repo; author runs Jekyll natively via rbenv)
...
- **Build process and requirements** – Native Ruby (rbenv 3.2.2) + Bundler workflow, dependency management
...
```bash
# Native Ruby workflow (the actual setup used in this repo)
# Requires rbenv-managed Ruby 3.2.2 and Bundler — already configured on the author's machine
bundle install                           # First time, or after Gemfile changes
bundle exec jekyll serve --livereload    # Auto-rebuild + auto-refresh browser
# Site available at http://localhost:4000/tmslab/  (note the /tmslab/ baseurl)
# Ctrl+C to stop
```
...
```bash
# One-shot production build (rarely needed manually)
...
> The `Dockerfile` and `docker-compose.yml` files in the repo are inherited from upstream al-folio and are NOT used here. Don't suggest `docker compose` commands — the author has no Docker engine installed.
...
1. **Run locally with Jekyll** (native Ruby workflow):

   ```bash
   bundle exec jekyll serve --livereload
   ```

   Then open `http://localhost:4000/tmslab/` in your browser. (If you get `bundle: command not found`, open a new terminal so rbenv loads.)
...
- 🚫 **Never do:**
  ...
  - Modify Docker configuration (these files are upstream al-folio cruft, not used here)
...
- Local preview with `bundle exec jekyll serve --livereload` runs on `http://localhost:4000/tmslab/`
...
| Test changes locally    | `bundle exec jekyll serve --livereload`                             | README.md § Running the site locally |
