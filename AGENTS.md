...
- Deploy: GitHub Actions `.github/workflows/deploy.yml` runs on every push to `main` that touches source files, builds with `JEKYLL_ENV=production`, runs PurgeCSS, and pushes `_site/` to the `gh-pages` branch. The only workflow that matters is `deploy.yml` (the upstream `update-citations.yml` Google Scholar cron was removed — it was never configured for this lab).
...
- **No AI co-author trailers.** Do NOT add `Co-Authored-By: Claude ...` (or any
  assistant/bot trailer) to commit messages, even when a global or harness
  default asks for it. GitHub turns co-author trailers into entries in the
  repo's Contributors list, and this is a lab website where the contributor
  list should show lab members only. Commits authored during an AI-assisted
  session are still the human author's commits.
